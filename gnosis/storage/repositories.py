...
from gnosis.core import Candidate, Relation, State, TransitionRecord
...
def _reject_secrets(value: Any, path: str = "payload") -> None:
    ...
    elif isinstance(value, str) and _SECRET_VALUES.search(value):
...
def _decode_payload(payload: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != {"elements", "version"} or not isinstance(value["version"], int):
        raise StorageCorruptionError("invalid state payload structure")
    return value

def save_state(conn: sqlite3.Connection, state: State, *, created_at: str | None = None) -> None:
    created_at = created_at or utc_now(); payload = state_payload(state)
    ...
    for order, relation in enumerate(state.relations):
        value = None if relation.value is None else canonical_json(relation.value)
        conn.execute("INSERT INTO relations(state_id,relation_order,relation_id,source_id,target_id,relation_type,value,created_at) VALUES(?,?,?,?,?,?,?,?)", (state.state_id,order,relation.relation_id,relation.source,relation.target,relation.relation_type,value,created_at))
...
def save_candidate(conn: sqlite3.Connection, candidate: Candidate, *, created_at: str | None = None) -> None:
    created_at=created_at or utc_now(); load_state(conn,candidate.parent_state_id); save_state(conn,candidate.proposed_state,created_at=created_at)
    ...
    expected=(candidate.parent_state_id,candidate.proposed_state.state_id,candidate.origin,candidate.seed)
...
def append_audit(conn: sqlite3.Connection, *, actor: str, action: str, resource: str, result: str, timestamp: str | None = None, event_key: str | None = None, transition_id_value: str | None = None) -> str:
    timestamp=timestamp or utc_now(); key=event_key or hashlib.sha256(canonical_json({"actor":actor,"action":action,"resource":resource,"result":result}).encode()).hexdigest()
    existing=conn.execute("SELECT event_hash,actor,action,resource,result,transition_id FROM audit_events WHERE event_id=?",(key,)).fetchone()
    if existing:
        ...
        return existing[0]
    last=conn.execute("SELECT sequence,event_hash FROM audit_events ORDER BY sequence DESC LIMIT 1").fetchone(); sequence=1 if last is None else int(last[0])+1; prev=GENESIS_HASH if last is None else last[1]
    event={"event_id":key,"sequence":sequence,"transition_id":transition_id_value,"actor":actor,"action":action,"resource":resource,"result":result,"timestamp":timestamp,"prev_hash":prev}; event_hash=_audit_hash(event)
    conn.execute("INSERT INTO audit_events(event_id,sequence,transition_id,actor,action,resource,result,timestamp,prev_hash,event_hash) VALUES(?,?,?,?,?,?,?,?,?,?)",(key,sequence,transition_id_value,actor,action,resource,result,timestamp,prev,event_hash))
    return event_hash
...
def save_instance(conn: sqlite3.Connection, instance: Instance, *, actor: str | None = None) -> None:
    with transaction(conn):
        save_state(conn,instance.engine.state); b=instance.engine.budget
        conn.execute("INSERT INTO instances(instance_id,parent_instance_id,owner_id,root_state_id,current_state_id,generation,status,budget_total,budget_spent,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",(instance.instance_id,instance.parent_instance_id,instance.owner_id,instance.engine.state.state_id,instance.engine.state.state_id,instance.generation,instance.status.value,b.total,b.spent,instance.created_at))
...
def load_instance(conn: sqlite3.Connection, instance_id: str) -> Instance:
    ...
    budget=Budget(total=row[7],spent=row[8]); instance=Instance(row[0],row[1],Engine(load_state(conn,row[3]),budget=budget),row[4],row[5],InstanceStatus(row[6]),row[9]); return instance

def recover_instance(conn: sqlite3.Connection, instance_id: str) -> Instance:
    """Fail-closed recovery entry point; low-level load_instance is not recovery validation."""
...
def verify_durable_graph(conn: sqlite3.Connection) -> tuple[int,str]:
    ...
    for row in conn.execute("SELECT instance_id,root_state_id,current_state_id,parent_instance_id,generation FROM instances"):
        ...
        if conn.execute("SELECT 1 FROM audit_events WHERE action='instance.create' AND resource=?", (row[0],)).fetchone() is None:
...
def persist_transition(conn: sqlite3.Connection, instance: Instance, candidate: Candidate, record: TransitionRecord, *, actor: str, failure_at: str | None = None) -> None:
    ...
    with transaction(conn):
        inject("after_begin")
        db=load_instance(conn,instance.instance_id)
        ...
        conn.execute("INSERT INTO transitions(transition_id,instance_id,candidate_id,from_state_id,to_state_id,accepted,reasons,created_at) VALUES(?,?,?,?,?,?,?,?)",(tid,instance.instance_id,candidate.candidate_id,record.from_state_id,record.to_state_id,int(record.accepted),canonical_json(record.test_result.reasons),utc_now()))
        inject("after_transition")
        append_audit(conn,actor=actor,action="transition.commit" if record.accepted else "transition.reject",resource=instance.instance_id,result="accepted" if record.accepted else "rejected",event_key=f"transition:{tid}",transition_id_value=tid)
        inject("after_audit")
        if record.accepted:
            conn.execute("UPDATE instances SET current_state_id=?,budget_total=?,budget_spent=? WHERE instance_id=?",(record.to_state_id,instance.engine.budget.total,instance.engine.budget.spent,instance.instance_id))
            inject("after_head")
